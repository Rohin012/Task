...
display= (data)=>{
    const displayHtml = data.map((v,index)=> {
        ...
        <div class="products-container" id="products">
          ...
          <article class="product">
            <img
              src="https://img.freepik.com/free-photo/isometric-miniature-plaza-de-toros-la-malagueta-malaga_23-2150101297.jpg?size=626&ext=jpg"
    ...
    let button = company.map((p)=>{
      return ` <button class="company-btn" class="company-btn1" onclick='FilterList("${p}")'>${p}</button>`
    })
...
searchCompany=()=>{
 ...
 Asc = ()=> {
  ...
  const Asc = products.sort((a, b)=>{
      if( a.company.length> b.company.length){
        return 1
      }else if(a.company.length== b.company.length){
 ...
 FilterList = (company) => {
  ...
  const Company = products.filter((v)=>{
